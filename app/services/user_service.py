...
from app.schemas.user_schema import UserSchema  # Importando o schema
import db
...
class UserService:
    ...
    def _validate_data(self, data):
        """
        Valida os dados do usuário e retorna os dados validados.
        """
        try:
            validated_data = UserSchema(**data)  # Valida com o schema
            return validated_data.dict()  # Retorna os dados validados como um dicionário
        except Exception as e:
    ...
    def _prepare_user_data(self, validated_data, user_id=None):
        ...
        """
        password_hash, formatted_cpf, formatted_birth_date, valid = self._validate_user_data(validated_data)
        if not valid:
